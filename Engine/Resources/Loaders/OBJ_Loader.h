		...
		static SBR::OBJ* Load(char* path)
		{
			...
			while (cur < eof)
			{
				if (cur[0] == 'v' && cur[1] == ' ')
				{
					...
					curMesh->positions.push_back(SBM::Vector3(x, y, z));
				}
				else if (cur[0] == 'v' && cur[1] == 't')
				{
					cur += 3;
					sscanf(cur, "%f %f", &x, &y);
					curMesh->uvs.push_back(SBM::Vector2(x, y));
				}
				else if (cur[0] == 'v' && cur[1] == 'n')
				{
					cur += 3;
					sscanf(cur, "%f %f %f", &x, &y, &z);
					curMesh->normals.push_back(SBM::Vector3(x, y, z));
				}
